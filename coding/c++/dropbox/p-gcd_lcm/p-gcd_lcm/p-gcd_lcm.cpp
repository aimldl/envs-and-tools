// p-gcd_lcm.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
int main() {
	...
	for (int i = 0; i < N; i++)
		multiples[i] = new int[size];

	// Multiples
	int M;  // for the size of array multiples
	for (int i = 0; i < N; i++)
	{
		M = get_multiples(num[i], multiples[i], size);
	}
	if (debug)
	{
		cout << "Multiples" << endl;
