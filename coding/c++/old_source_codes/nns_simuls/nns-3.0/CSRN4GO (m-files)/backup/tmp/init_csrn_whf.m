function CSRNwHF=init_csrn_whf(board),
% init_csrn_whf   initialize CSRN with hidden layer feedback
%
...
CSRNwHF.cfg.N_e         = CSRNwHF.cfg.N_h+2;          % Number of input elements to the hidden layer
...
CSRNwHF.srn.W_U         = zeros( N_h,N_b.*N_u );
CSRNwHF.srn.W_h         = zeros( N_h,N_e  );
...
% commented out.
% CSRNwHF.srn.w_u_start = 1
CSRNwHF.srn.w_u_end     = N_h.*N_u;
% CSRNwHF.srn.w_h_start = CSRNwHF.srn.w_u_end+1;        
CSRNwHF.srn.w_h_end     = CSRNwHF.srn.w_u_end+N_h.*(N_h+2);
% CSRNwHF.srn.w_v_start = CSRNwHF.srn.w_h_end+1;
% CSRNwHF.srn.w_v_end   = the size of the vector;

% Hidden layer output
CSRNwHF.srn.H_o         = rand(N_h,1);
